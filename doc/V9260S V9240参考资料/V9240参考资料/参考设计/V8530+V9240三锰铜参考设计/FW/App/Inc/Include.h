...
typedef struct
{
    uint32 ZXDatReg;//电压过零点前一次采样值
    uint32 ZXDat;	//电压过零点当前点采样值
    uint32 PHDat;	//电压相位数据
    uint32 NoDef;	
    uint32 T8Baud;
...
typedef struct
{
    uint16  uc_80msCnt;            // 小信号有功80MS  
    uint16  uc_80msCntRE;      //小信号无功80ms
    uint16  uc_160msCnt;         //大信号有功160ms
    uint16  uc_160msCntRE;   //大信号无功160ms
    uint8  uc_1280msCnt;     //秒平均计数
}S_IMPTIM;
...
VAREXT uint8 Ioawkup_flag,Rtcwkup_flag;

//VAREXT uint8 guc_RTCSave;//zzp1203
#endif
