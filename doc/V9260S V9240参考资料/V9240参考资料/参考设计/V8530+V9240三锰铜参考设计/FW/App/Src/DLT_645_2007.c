...
//extern uint8 guc_RTCSave;//zzp1203
...
const Gs_VarType Gs_RdVarTab[]=
{
    ...
    {PAAVG/*DATAQ*/,     0x04,   0x01,       0x03,   },      //17 无功功率数据块
    {PAAVG/*DATAQ*/,     0x00,   0x01,       0x03,   },      //18 无功A功率
    {PAAVG/*DATAQ*/,     0x01,   0x01,       0x03,   },      //19 无功B功率
    {PAAVG/*DATAQ*/,     0x02,   0x01,       0x03,   },      //20 无功C功率
...
#endif               
            if(Ret_Err == Report(s_FrmInfo, (S_CORETAB*)gs_CoreTab4 + 0x32))
            {
                return Ret_Err;
            }
        }else
...
extern  uint8 InitPara5[sizeof(S_JBPM)-2];
/*=========================================================================================\n
...
//   uint32 temp;
  ...
  else if(s_FrmInfo->uiID.lword==0x4F520002)     //自动校表  1%Ib 1.0
  {
    ...
    else if(s_FrmInfo->uiID.lword==0x20000001)                          //满度初始化  zyj 1112 需要上位机配合调试
    {
        ...
        memcpy((uint8*)&gs_JbPm.ui_MeterC,s_FrmInfo->ucbuf+4,2);
        memcpy((uint8*)&gs_JbPm.ui_Un,s_FrmInfo->ucbuf+6,2);
        memcpy((uint8*)&gs_JbPm.ui_Ib,s_FrmInfo->ucbuf+8,2);
        //上面一次传6个字节改为一次传2字节分3次到 zzp1201
        
        memcpy((uint8*)&gs_JbPm.RacEGYTH,s_FrmInfo->ucbuf+10,8);           //有功门限制、无功门限制 

        //memcpy((uint8*)&gs_JbPm.RacEGYTH,s_FrmInfo->ucbuf+10,8);
        ...
        /********************************************zzp1202调试用
        BE_ReadP(0x01B8, &uitmpEE,1);
        BE_ReadP(0x01B9, &uitmpEE,1);
        BE_ReadP(0x01Ba, &uitmpEE,1);
        BE_ReadP(0x01Bb, &uitmpEE,1);
        uitmp=do_CRC((uint8*)&gs_JbPm,sizeof(S_JBPM)-2);
        BE_ReadW(EEP_JBTOTAL+sizeof(S_JBPM)-4, &uitmp);
        BE_ReadW(EEP_JBTOTAL+sizeof(S_JBPM)-3, &uitmp);
        BE_ReadW(EEP_JBTOTAL+sizeof(S_JBPM)-2, &uitmp);
        BE_ReadW(EEP_JBTOTAL+sizeof(S_JBPM)-8, &uitmp);
        BE_ReadP(0x0140, XDATA_RAMZONE2,125);
        uitmp=do_CRC((uint8*)&XDATA_RAMZONE2,120);
        uitmp=do_CRC((uint8*)&XDATA_RAMZONE2,120);
        uitmp=do_CRC((uint8*)&XDATA_RAMZONE2,124);
        uitmp=do_CRC((uint8*)&XDATA_RAMZONE2,124);
...
//        gui_RefreshEvent |= flgEtPara_EnyBottom;
    ...
    else if(s_FrmInfo->uiID.lword==0x2000000a)                  //电流有效值小信号二次补偿值
    {
        gs_JbPm.gs_JBA.RacIADCC=Uint8_To_Uint32(s_FrmInfo->ucbuf+4);  //二次补偿值
        gs_JbPm.gs_JBB.RacIADCC=Uint8_To_Uint32(s_FrmInfo->ucbuf+8);  //二次补偿值
        gs_JbPm.gs_JBC.RacIADCC=Uint8_To_Uint32(s_FrmInfo->ucbuf+12); 
        gs_JbPm.ui_JbCRC=do_CRC((uint8*)&gs_JbPm,sizeof(S_JBPM)-2);     // 校表参数的CRC结果
        SysE2ParaSetManage(EEP_JBTOTAL,(uint8*)&gs_JbPm,sizeof(S_JBPM)); //写入E2中
        gui_RefreshEvent |= flgEtPara_EnyBottom;
    }
   ...
   pVar = (Gs_VarType*)&Gs_RdVarTab + CTable->uiOther+ucpos;
    if(pVar->Atb==0x01)
    {
        if(s_FrmInfo->ucAtb & Const_Atb_DataJC)     //如果是集抄的话
        {
            ...
            else if(pVar->ulAdrr == PAAVG)	//else if(pVar->ulAdrr == DATAQ)
            {
                ulVarValue = CalRMS2(PAAVG/*DATAQ*/,(gs_power.ul_Npower[0] + gs_power.ul_Npower[1] + gs_power.ul_Npower[2]));
                memcpy(s_FrmInfo->ucbuf,(uint8*)&ulVarValue,pVar->ucLen);
                for(i=0;i<3;i++)
                {
                    ulVarValue = CalRMS2(PAAVG/*DATAQ*/,((gs_power.ul_Npower[i])));
                    memcpy(s_FrmInfo->ucbuf+pVar->ucLen*(i+1),(uint8*)&ulVarValue,pVar->ucLen);
                }
                s_FrmInfo->ucLen=pVar->ucLen*4;
                return Ret_OK;
        ...
        else
        {
            ...
            {
              if(CalRMS2(PAAVG,gs_power.ul_power[pVar->phy]) || CalRMS2(PAAVG/*DATAQ*/,gs_power.ul_Npower[pVar->phy]))  //功率为零，电流为零
              {
            ...
            else if(pVar->ulAdrr == PAAVG/*DATAQ*/)
            {
                switch(pVar->phy)
                {
                    case 0x00:
                    case 0x01:
                    case 0x02:
                        ulVarValue = CalRMS2(PAAVG/*DATAQ*/,(gs_power.ul_Npower[pVar->phy]));
                        break;
                    case 0x04:
                        ulVarValue = CalRMS2(PAAVG/*DATAQ*/,(gs_power.ul_Npower[0] + gs_power.ul_Npower[1] + gs_power.ul_Npower[2]));
                        break;
