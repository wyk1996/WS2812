...
{                                                        
  ...
  //  ↑1122
    Init_Adjparameter((uint8 *)&gs_JbPm.ui_MeterC,InitPara5);
...
{
    //把RAM中的校表参数跟新到SFR中
    //EnyB_SetMeterCfgACK(gs_JbPm.RacEGYTH/3,0x10F4); //E1门限    zzp1130
    //EnyB_SetMeterCfgACK(gs_JbPm.RacREEGYTH/3,0x10FA); //E2门限  zzp1130
    Clashin();
    UpdateRaccoonPara();
    BroadcastWriteRaccoon( AnaCtrl1_Val, 0x0183); //CSEL<1：0>设置为01
    //*****CSEL调整I通道adc密勒电容 01 增加33%（推荐01）
    CheckRegSum();
    CLRWDT(); 
//    CheckSysErr();  
    Clashout();
