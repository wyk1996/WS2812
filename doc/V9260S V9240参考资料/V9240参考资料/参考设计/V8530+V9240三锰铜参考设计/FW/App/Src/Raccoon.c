...
* @修改内容: 
===========================================================================================*/
void RxReset_V9260(void)		//	V9240rx复位
{
    PHACtrlEnable();
    PHBCtrlEnable();
    PHCCtrlEnable();
    ...
    //P2OD |=(BIT1);
    //P2OD&=(~BIT1);  		//MCUtx引脚输出低电平使计量rx脚输出70ms低电平使计量芯片复位

	
	GPIOB->OEN &=~ BIT7; //使能tx脚输出功能
	GPIOB->IE &=~ BIT7; //禁止tx脚输入功能

	GPIOB->DAT &=~ BIT7;//TX输出低电平
	
    guc_CommDelayTime = 10;
    while(guc_CommDelayTime >0);	//延时100ms
    ////DelayXms(200);
      //P2OD |=(BIT1);
    GPIOB->DAT |= BIT7;//tx恢复高电平
...
//    gs_RacCtrl.ucBuf[0] = 0xfe;    //帧头 0x7D
...
//                gs_UI.ul_Hz[0] =(uint32)HZCONST/TempValue2.lword;
            ...
            }else
            {
                ...
                //UART_SendData( UART0, gs_power_enerycal.ul_power[0]);
            }
...
//                gs_UI.ul_Hz[1] =TempValue2.lword;//(uint32)HZCONST/TempValue2.lword;
            ...
            }else
            {
                ...
                //UART_SendData( UART0, gs_power_enerycal.ul_power[1]);
            }            
...
//                UART0->DATA = (((gs_power_enerycal.ul_power[2])>>8)&0xff);
//                UART0->DATA = ((gs_power_enerycal.ul_power[2])&0xff);
...
void CheckRegSum(void)
{
    ...
    ucSum = 
          ...
          + gs_JbPm.gs_JBA.RacIADCC//zzp0131
          + AnaCtrl1_Val;
    ...
    ucSum = 
          ...
          + gs_JbPm.gs_JBB.RacIADCC//zzp0131
          + AnaCtrl1_Val;
    ...
    ucSum = 
          ...
          + gs_JbPm.gs_JBC.RacIADCC//zzp0131
          + AnaCtrl1_Val;
           ...
           CLRWDT(); 
            ...
            if(i < 2)  //SysCtrl,BPFPARA
            {
              if( i == 0) //SysCtrl
              {
                ...
                if(tmp ==false)
                {
                   tmp = WriteRaccoon(SysCtrl_Value[j],RegAddr[i], phadd[j]);
...
//                    ucSum += ul_pRacReg[i-2];
        ...
        switch(phadd[j])   //电流有效值小信号二次补偿 
        {
            case APhy:
                ul_pRacReg = (uint32*)&gs_JbPm.gs_JBA.RacIADCC;
                break;
            case BPhy:
                ul_pRacReg = (uint32*)&gs_JbPm.gs_JBB.RacIADCC;
                break;  
            case CPhy:
                ul_pRacReg = (uint32*)&gs_JbPm.gs_JBC.RacIADCC;
                break;
        }
        tmp = WriteRaccoon(*ul_pRacReg, IADCC, phadd[j]); 
        if(tmp ==false)
        {
          WriteRaccoon(*ul_pRacReg, IADCC, phadd[j]);  
        }
        CLRWDT();
