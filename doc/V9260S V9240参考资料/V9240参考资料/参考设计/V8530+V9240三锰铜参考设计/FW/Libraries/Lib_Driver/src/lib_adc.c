...
/**
  * @brief  Get VDD Voltage(takes 170us).
...
uint32_t ADC_GetVDDVoltage_Fast(uint32_t Division, ADC_CalResType *CalResults)
{
  ...
  {
    CalResults->VDDVoltage = (((int16_t)ANA->ADCDATA1 - 49.588) / 7088.91575) + Vbatcap;
  }
  else
  {
    CalResults->VDDVoltage = (0.00015392*(int16_t)ANA->ADCDATA1 + 0.06667986) + Vbatres;
  }
...
/**
  * @brief  Get BATRTC Voltage(takes 170us).
  * @note   This function costs about 170us when SystemClock is 26M.
  *         ADC data refresh time is 40~50us.
  * @note   This function will release ADC resource(write ADC registers with their
  *         default reset values).
...
/**
  * @brief  Pull-down resistance(1M) for input signal.
  * @param  NewState
                ENABLE
                DISABLE
  * @retval None
