...
uint32_t PMU_EnterDSleepMode(void)
{
  ...
  ANA->REGA |= ANA_REGA_SWT2BAT1 | ANA_REGA_VDD3_OFF;
...
/**
  ...
  * @note   This function performs the following:
                ...
                Disable AHB/APB periphral clock       Modules(optional)
                Disable AVCC output
                  2: VDCIN is not drop before enter sleep mode(failed).
                1: Current mode is debug mode, enter sleep mode f Power down ADC, Power down Temp sensor
                Disable resistor/cap division for ADC input signal
