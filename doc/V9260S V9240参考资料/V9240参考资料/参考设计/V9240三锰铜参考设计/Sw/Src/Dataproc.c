...
void Data_ClearMeter(void)
{
    ...
    if(CheckCRCWord(guc_ClearWd))
    {
        ...
        Dsip_Clr();
        //电表清零记录
       // if(guc_FactoryType!=FactorMd)           //农网厂内清零，记录清零事件
       // {
      ...
      //  }
        ...
        for(i=0;i<13;i++)                                       //清历史电量
        {
           ...
           // BE_WriteP(EEP_EPHISWG_PT+64*i,XDATA_RAMZONE,64);      //历史电量            
        }
     ...
     /*  for(i=0;i<dim(gCs_DJClearAddrTab);i++)                  //清冻结数据
        ...
        //else
       {
           /* for(i=0;i<dim(gCs_MeterClearAddrTab);i++)
            ...
            //密码闭锁取消
          //  MemSet((uint8*)&gs_PassWordCtr.uiDyPW2Out,0x00,sizeof(S_PWCTR));
